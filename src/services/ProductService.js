...
const createProduct = (newProduct) => {
    return new Promise(async (resolve, reject) => {
        const { name, images, type, stock, price, rating, description, reviews, colors, sizes, discount,selled } = newProduct
    ...
    return new Promise(async (resolve, reject) => {
        try {
            ...
            if (filter) {
                ...
                const allProductFilter = await Product.find({ [label]: { '$regex': filter[1] } }).sort({createdAt: -1, updatedAt: -1})
